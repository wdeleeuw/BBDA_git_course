...
# Getting started
Git is a distributed version tracking system with broad functionality and many options. A basic hands on introduction using Crunchomics somewhat targeted to bioinformatics projects is given here.   
 
Log in to Crunchomics. Git is installed on the system however it has to be configured for a user once using the following commands: 

```
git config --global user.name  "put your name here"
git congit --global user.email "put your email here"
```
...
If 
```
  git clone <UvAnetID>@omics-h0.science.uva.nl:<path_to_repository>
```
  

# Parallel development 

To pick up development which was done on  
```
git fetch master
git merge
``
...
Up to now we we used a local github repository hosted in a directory on Crunchomics. Access is possible only for people with access Crunchomics in general and the directory in particular.  Github is a company (Microsoft) which hosts github repositories. Personal and public repositories are free. Non public multi user repositories are not. By creating an account on github you can store your repositories accessible from any location and make it available world wide. 

http://github.com

In R the command `install_github()` from the devtools package Can be used to install R packages which are hosted on github. 
...
-   __tab__: complete filename. Press twice for options 
-   __up and down arrow keys__: browse command history
-   __ctrl-c__: terminate running program
-   __ctrl-z__: stop running program. fg/bg to resume in foreground or background
-   __ctll-d__: end of file
